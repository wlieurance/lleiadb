...
filter.table <- function(in.tbl, val, filter.str){
  filter.matrix <- (filter.str |>
    stringr::str_match_all("([^,;\\s]+)[\\s,]*([^,;\\s]+)*[\\s;$]*"))[[1]]

  if (nrow(filter.matrix) > 0){
    filt.tbl <- foreach::foreach(i = 1:nrow(filter.matrix),
                         .combine = dplyr::bind_rows) %do% {
      if (is.na(filter.matrix[i, 3])){
        filt.tbl <- in.tbl |> dplyr::filter(indicator == filter.matrix[i, 2])
      } else {
        filt.tbl <- in.tbl |> dplyr::filter(indicator == filter.matrix[i, 2] &
  ...
  fnames <- sort(names(filt.wide)[!names(filt.wide) %in% c("plotkey",
                                                          "survey_year",
                                                          "survey_date_min")])
  filt.wide <- filt.wide |> dplyr::select(plotkey, survey_year, survey_date_min,
...
write.spatial <- function(tbl.out, out.path, dbname, user, password, host,
  ...
  final.tbl <- site |>
    dplyr::select(sitekey, siteid, site_name, source, source_type) |>
    dplyr::inner_join(dplyr::select(point, sitekey, plotkey, plotid),
...
lpi.to.gis <- function(
  ...
  tbl.head <- in.tbl |>
    dplyr::group_by(plotkey, survey_year, survey_date_min) |>
    dplyr::summarize(line_n = max(cvr_n), .groups = "drop")

  if (!is.null(cover.str)){
    filt.tbl.cvr <- filter.table(in.tbl = in.tbl, val = "cover",
                                 filter.str = cover.str)
    tbl.head <- tbl.head |>
      dplyr::left_join(filt.tbl.cvr, by = c("plotkey" = "plotkey",
                                     "survey_year" = "survey_year",
                                     "survey_date_min" = "survey_date_min"))
  }
  ...
  if (!is.null(height.str)){
    ...
    tbl.head <- tbl.head |>
      dplyr::left_join(filt.tbl.hgt, by = c("plotkey" = "plotkey",
                                     "survey_year" = "survey_year",
                                     "survey_date_min" = "survey_date_min"))
  }
  ...
  if (!is.null(dead.str)){
    filt.tbl.dead <- filter.table(in.tbl = in.tbl, val = "dead",
                                 filter.str = dead.str)
    tbl.head <- tbl.head |>
      dplyr::left_join(filt.tbl.dead, by = c("plotkey" = "plotkey",
                                     "survey_year" = "survey_year",
                                     "survey_date_min" = "survey_date_min"))
  }
...
if (sys.nframe() == 0) {
  ...
  option_list = list (
    ...
    optparse::make_option(opt_str = c("-S", "--srid"),
                ...
                default = 4326),
    optparse::make_option(opt_str = c("-s", "--sep"),
