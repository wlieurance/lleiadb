...
get.dest.tables <- function(schema){
  ...
  tables <- DBI::dbGetQuery(con, tables.sql)
...
get.dest.info <- function(schema, table){
  info.sql <- glue::glue(paste(
    ...
    sep = "\n"))
  col.info <-  DBI::dbGetQuery(con, info.sql)

  constraint.sql = glue::glue(paste(
    ...
    sep = "\n"))
  constraints <-  DBI::dbGetQuery(con, constraint.sql)
...
get.src.info <- function(tbl){
  ...
  info <- col.names |>
    dplyr::left_join(src.types, by=c("column_name" = "variable")) |>
...
compare.types <- function(src.cols, dest.cols){
  ...
  src.cols <-  src.cols |> dplyr::rename(src_type = data_type)
  dest.cols <-  dest.cols |> dplyr::rename(dest_type = data_type)
  compare <- dest.cols |>
    dplyr::inner_join(src.cols, by = c("column_name" = "column_name")) |>
    dplyr::left_join(type.match, by = c("dest_type" = "ptype")) |>
    dplyr::mutate(matched = stringr::str_detect(rtype, src_type)) |>
    dplyr::mutate(matched = ifelse(is.na(matched), FALSE, matched))
  return (compare)
}
...
insert.row <- function(row, stmt, log = NULL){
  ...
  error = function(e) {
    ...
    e.short <- stringr::str_replace(e$message,
    ...
    a <- tryCatch(
      {
        ...
        DBI::dbBind(send, row)
        rw.bind.affected <- DBI::dbGetRowsAffected(send)
...
dbbind.insert <- function(update, schema, table, table.name, cols, cast, log,
  ...
  while (processed < tbl.nrow) {
    ...
    sub.tbl <- dplyr::slice(table, (processed + 1):(end.row))
    send.data <- as.list(dplyr::select(sub.tbl, insert$cols))
    names(send.data) <- NULL
    affected <- tryCatch(
      {
        send <- DBI::dbSendStatement(con, insert$sql)
        DBI::dbBind(send, send.data)
        bind.affected <- DBI::dbGetRowsAffected(send)
        DBI::dbClearResult(send)
...
to.db <- function(tbls, update, dbkey, desc, path, hash, verbose = FALSE,
  ...
  for (schema in schemas){
    ...
    order.source <- order |>
      dplyr::inner_join(tibble::as_tibble(list(tblname=table.names)),
                                         by=c("tblname" = "tblname")) |>
      dplyr::mutate(level = ifelse(tblname == "db", -1, level)) |>
      dplyr::arrange(level, tblname)
...
get.src.tables <- function(path, md5hash, key, desc = NULL){
  ...
  dima.tbls.sql <- paste0("SELECT table_name FROM information_schema.tables ",
                          ...
                          "'tblMaint%';")
  dima.table.names <- (DBI::dbGetQuery(con, dima.tbls.sql))$table_name
  lmf.tbls.sql <- paste0("SELECT table_name FROM information_schema.tables ",
                         "WHERE table_schema='lmf';")
  lmf.table.names <- (DBI::dbGetQuery(con, lmf.tbls.sql))$table_name
  ...
  for (schema in process.schemas){
    ...
    for (name in tbl.names){
      ...
      char.fields <- dest.info$col.info |>
        dplyr::filter(!is.na(character_maximum_length))
...
if (sys.nframe() == 0) {
  option_list <-  list (
    ...
    optparse::make_option(opt_str = c("-u", "--update"), action = "store_true",
                default = FALSE,
    ...
    optparse::make_option(opt_str = c("-k", "--key"),
                help = paste0("A short/unique code for this database. [Will be",
                              " generated automatically if not provided].")),
    optparse::make_option(opt_str = c("-d", "--desc"),
                help = paste0("Descriptive text for documenting the source ",
                              "data.")),
    optparse::make_option(opt_str = c("-l", "--log"), action = "store_true",
                default = FALSE,
                help = paste0("Log the results of the import. Stored in the ",
                              "executing directory as ",
                              "'import_YYYYMMDDHHMMSS.log' ",
                              "[default: %default]")),
    optparse::make_option(opt_str = c("-v", "--verbose"), action = "store_true",
                default = FALSE,
                help = paste0("Increase the level of script reporting ",
                "[default: %default].")),
    optparse::make_option(opt_str = c("-s", "--save_raw"),
                help = paste0("Path at which to save a raw version of the ",
                              "imported tables in RDS format.")),
    optparse::make_option(opt_str = c("-S", "--save_processed"),
    ...
    optparse::make_option(opt_str = c("-c", "--chunk_size"), default = 1000,
                help = paste0("The number of rows to insert into the db at ",
                              ...
                              "[default: %default].")),
    optparse::make_option(opt_str = c("-r", "--skip_refresh"),
                          action = "store_true",
                default = FALSE,
  ...
  opt_parser = optparse::OptionParser(
    usage = "usage: %prog [options] dbname user source_data",
